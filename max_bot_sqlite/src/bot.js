...
const BOT_SEARCH_EXAMPLES = 'Логотип, Брендбук, Город, Паттерн, Шрифт, Сувенир';

function buildMainMenuText(intro = false) {
  if (intro) {
    return [
      'Привет! Добро пожаловать в официальный каталог бренда Ямала.',
      'Этот чат-бот создан для удобства государственных служащих и предпринимателей, которые хотят использовать элементы официального бренда региона в своей деятельности.',
      'В каталоге вы найдете утвержденные материалы, включая брендбуки, логотипы, шрифты и паттерны, которые помогут в реализации проектов в регионе и продвижении бизнеса.',
      'Можно открыть верхние разделы каталога, перейти к городским брендбукам и паттернам, добавить материалы в Избранное или использовать Поиск.',
      `Просто отправьте текст: ${BOT_SEARCH_EXAMPLES}.`,
    ].join('\n\n');
  }
...
function buildSearchText() {
  return [
    ...
    'Можно искать брендбуки, логотипы, шрифты, паттерны, городские материалы и сувенирную продукцию.',
