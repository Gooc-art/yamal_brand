...
- UX помощника тоже держи консистентным: ответы должны жить внутри отдельного диалогового окна с собственной прокруткой, переходы по кнопкам из ответа не должны закрывать сам диалог, а на ПК он должен уходить только по явному действию `Закрыть` или `Esc`.
