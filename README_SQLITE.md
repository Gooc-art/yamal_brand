...
Точные категорийные запросы `логотип`, `шрифт`, `паттерн` и `иллюстрация` возвращают только актуальные папки этой категории. Файлы внутри открываются через найденную папку, поэтому выдача не дублирует каждый файл из-за упоминания категории в пути.
