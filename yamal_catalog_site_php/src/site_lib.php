...
function constructor_color_variant_labels(): array
{
    return [
        'cmyk' => 'CMYK для печати',
        'color' => 'Color',
        'black' => 'Black',
        'white' => 'White',
    ];
...
function constructor_brand_lockup_labels(): array
{
    return [
        'logo' => 'Логотип с надписью',
        'mark' => 'Фирменный знак',
    ];
}
...
function constructor_background_style_labels(): array
{
    return [
        'clean' => 'Чистый фон',
        'band' => 'Сигнальная плашка',
        'frame' => 'Контур и рамка',
...
{
    return [
        ['value' => 'calm', 'label' => 'Спокойный', 'description' => 'Больше воздуха и чистый ритм', 'mark' => 'CALM'],
...
function constructor_style_fields(array $defaults = []): array
{
    ...
    return [
        ...
        'design_variant' => [
            ...
            'options' => constructor_design_variant_options(),
        ],
        'background_style' => [
            'id' => 'background_style',
            'type' => 'select',
    ...
    return match ((string) ($definition['id'] ?? '')) {
        ...
        'presentation_deck' => [
            ...
            [
                ...
                'overrides' => ['presentation_mode' => 'pitch', 'design_variant' => 'signal', 'background_style' => 'corner', 'palette_tone' => 'accent', 'brand_lockup' => 'mark'],
            ],
            [
                'id' => 'invest',
                'label' => 'Инвест',
        ...
        'badge' => [
            [
                ...
                'overrides' => ['access_level' => 'speaker', 'design_variant' => 'signal', 'background_style' => 'band', 'palette_tone' => 'accent', 'brand_lockup' => 'mark'],
            ],
            [
                'id' => 'staff',
                'label' => 'Команда',
        ...
        'social_post' => [
            [
                ...
                'overrides' => ['ratio' => '4:5', 'design_variant' => 'signal', 'background_style' => 'band', 'palette_tone' => 'accent', 'brand_lockup' => 'logo'],
            ],
            [
                'id' => 'editorial',
                'label' => 'Редакционный',
        ...
        'rollup' => [
            [
                ...
                'overrides' => ['design_variant' => 'signal', 'background_style' => 'watermark', 'palette_tone' => 'accent', 'brand_lockup' => 'logo'],
            ],
            [
                'id' => 'contrast',
                'label' => 'Контрастный',
...
function constructor_asset_contents(string $relativePath): string
{
    static $cache = [];
...
function constructor_asset_variant_data_uri(string $relativePath, string $mimeType, string $colorVariant): string
{
    static $cache = [];
    $cacheKey = $relativePath . '|' . $mimeType . '|' . $colorVariant;
    if (isset($cache[$cacheKey])) {
        return $cache[$cacheKey];
    }

    $content = constructor_asset_contents($relativePath);
    if ($content === '') {
        return '';
    }
...
{
    return [
        'logo' => constructor_asset_variant_data_uri('assets/brand-logo-main.svg', 'image/svg+xml', $colorVariant),
        'mark' => constructor_asset_variant_data_uri('assets/brand-mark.svg', 'image/svg+xml', $colorVariant),
    ];
}

function constructor_brand_asset_bundle(): array
{
    static $bundle = null;
    if (is_array($bundle)) {
        return $bundle;
    ...
    $variants = ['color', 'cmyk', 'black', 'white'];
    $bundle = [
        'logo' => [],
        'mark' => [],
    ];
    ...
    foreach ($variants as $variant) {
        $bundle['logo'][$variant] = constructor_asset_variant_data_uri('assets/brand-logo-main.svg', 'image/svg+xml', $variant);
        $bundle['mark'][$variant] = constructor_asset_variant_data_uri('assets/brand-mark.svg', 'image/svg+xml', $variant);
    }

    return $bundle;
}

function constructor_svg_variant_asset_attributes(array $variantAssets): string
{
    $out = '';
    foreach ($variantAssets as $variant => $href) {
        $normalizedVariant = preg_replace('/[^a-z0-9_-]+/i', '', (string) $variant) ?: 'color';
        $out .= ' data-href-' . $normalizedVariant . '="' . constructor_svg_escape((string) $href) . '"';
    }
    return $out;
}

function constructor_svg_image(string $href, float $x, float $y, float $width, float $height, float $opacity = 1.0): string
{
    if ($href === '') {
        return '';
    }
...
function constructor_svg_render_brand_lockup(
    float $x,
    float $y,
    string $brandLockup,
    array $brandAssets,
    array $brandAssetBundle = [],
    string $brandVariant = 'color',
    array $options = []
): string
{
    $useMark = $brandLockup === 'mark';
    $href = $useMark ? (string) ($brandAssets['mark'] ?? '') : (string) ($brandAssets['logo'] ?? '');
    if ($href === '') {
        return '';
    }

    $width = $useMark
        ? (float) ($options['markWidth'] ?? 176.0)
        : (float) ($options['logoWidth'] ?? 188.0);
    $height = $useMark
        ? (float) ($options['markHeight'] ?? round($width / 1.5391, 1))
        : (float) ($options['logoHeight'] ?? round($width / 6.1227, 1));

    $variantAssets = $useMark
        ? (array) ($brandAssetBundle['mark'] ?? [])
        : (array) ($brandAssetBundle['logo'] ?? []);
    $activeHref = (string) ($variantAssets[$brandVariant] ?? $href);
...
function constructor_svg_theme_style_vars(array $theme, array $lockupSurface = []): string
{
    $variables = [
        ...
        '--ctor-ink' => constructor_theme_value($theme, 'ink', '#182a31'),
        '--ctor-muted' => constructor_theme_value($theme, 'muted', '#5d6972'),
        '--ctor-surface-alt' => constructor_theme_value($theme, 'surfaceAlt', '#f6f0e7'),
...
function constructor_svg_document(float $width, float $height, string $body, array $theme, array $lockupSurface = []): string
{
    $styleVars = constructor_svg_escape(constructor_svg_theme_style_vars($theme, $lockupSurface));
    return '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ' . $width . ' ' . $height . '" width="' . $width . '" height="' . $height . '">' .
        '<defs><style><![CDATA[' .
        '.bg{fill:var(--ctor-bg);}' .
        '.accent{fill:var(--ctor-accent);}.accent-soft{fill:var(--ctor-accent-soft);}.tone{fill:var(--ctor-tone);}.tone-soft{fill:var(--ctor-tone-soft);}.ink{fill:var(--ctor-ink);}.muted{fill:var(--ctor-muted);}.sand{fill:var(--ctor-surface-alt);}.white{fill:#ffffff;}.tone-ink{fill:var(--ctor-tone-ink);}' .
        '.title{font:700 64px DejaVu Sans,Arial,sans-serif;fill:var(--ctor-ink);letter-spacing:-0.02em;}' .
        '.headline{font:700 92px DejaVu Sans,Arial,sans-serif;fill:var(--ctor-ink);letter-spacing:-0.03em;}' .
        '.subhead{font:600 36px DejaVu Sans,Arial,sans-serif;fill:var(--ctor-ink);}' .
        '.body{font:400 30px DejaVu Sans,Arial,sans-serif;fill:var(--ctor-ink);}' .
        '.small{font:400 24px DejaVu Sans,Arial,sans-serif;fill:var(--ctor-muted);}' .
        '.tiny{font:500 20px DejaVu Sans,Arial,sans-serif;fill:var(--ctor-muted);letter-spacing:0.04em;text-transform:uppercase;}' .
        '.badge{font:700 28px DejaVu Sans,Arial,sans-serif;fill:var(--ctor-badge);letter-spacing:0.08em;text-transform:uppercase;}' .
        '.line{stroke:var(--ctor-line);stroke-width:2;}' .
        '.card{fill:var(--ctor-surface);stroke:var(--ctor-card-stroke);stroke-width:2;}' .
        'text{white-space:pre;}' .
        ']]></style></defs>' .
        '<g data-constructor-preview-root="1" style="' . $styleVars . '">' .
...
function constructor_style_display_labels(array $input): array
{
...
function constructor_html_brief_artifact(array $definition, array $input, array $recommendations): array
{
    ...
    $html = '<!doctype html><html lang="ru"><head><meta charset="utf-8"><title>' .
        ...
        '<div class="card"><strong>Дизайн</strong><p>' . htmlspecialchars((string) ($derived['designVariantLabel'] ?? ''), ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8') . '</p></div>' .
...
function constructor_svg_artifact(array $definition, array $input): ?array
{
    ...
    switch ((string) ($definition['id'] ?? '')) {
        case 'business_card':
            ...
            if ($useTopBandLayout) {
                $body =
                    ...
                    constructor_svg_render_fitted_text(84, 236, (string) ($input['full_name'] ?? ''), 'headline', 732, 148, 4, ['minFontSize' => 22, 'widthSafety' => 0.86, 'heightSafety' => 0.88]) .
                    constructor_svg_render_fitted_text(84, 344, (string) ($input['role'] ?? ''), 'subhead', 732, 72, 2, ['minFontSize' => 16]) .
                    '<rect x="84" y="376" width="732" height="54" rx="18" fill="var(--ctor-tone-soft)"/>' .
                    constructor_svg_render_fitted_text(118, 410, trim((string) ($input['phone'] ?? '')) . ' • ' . trim((string) ($input['email'] ?? '')), 'small', 664, 26, 1, ['minFontSize' => 12]) .
                ...
                $body =
                    ...
                    constructor_svg_render_fitted_text($contentX, 158, (string) ($input['full_name'] ?? ''), 'headline', $contentWidth, 190, 4, ['minFontSize' => 22, 'widthSafety' => 0.86, 'heightSafety' => 0.88]) .
                    constructor_svg_render_fitted_text($contentX, 282, (string) ($input['role'] ?? ''), 'subhead', $contentWidth, 96, 3, ['minFontSize' => 16, 'widthSafety' => 0.88]) .
                    constructor_svg_render_fitted_text($contentX, 352, (string) ($input['department'] ?? ''), 'small', $contentWidth, 66, 2, ['minFontSize' => 14]) .
        ...
        case 'nameplate':
            ...
            $arrowPath = match ($direction) {
                ...
                'up' => '<path d="M984 318V222M984 222l-30 30M984 222l30 30" stroke="var(--ctor-accent)" stroke-width="20" stroke-linecap="round" stroke-linejoin="round" fill="none"/>',
                'down' => '<path d="M984 214v96M984 310l-30-30M984 310l30-30" stroke="var(--ctor-accent)" stroke-width="20" stroke-linecap="round" stroke-linejoin="round" fill="none"/>',
                default => '',
            };
                ...
                $body =
                    ...
                    ($roomNumber !== ''
                        ? '<rect x="286" y="138" width="164" height="138" rx="26" class="tone-soft"/>' .
                            constructor_svg_render_fitted_text(318, 228, $roomNumber, 'subhead', 100, 92, 2, ['minFontSize' => 16, 'heightSafety' => 0.82])
                        : '') .
                    constructor_svg_render_fitted_text($roomNumber !== '' ? 492 : 286, 182, (string) ($input['location'] ?? ''), 'headline', $roomNumber !== '' ? 482 : 688, 164, 4, ['minFontSize' => 20, 'widthSafety' => 0.84, 'heightSafety' => 0.86]) .
                    constructor_svg_render_fitted_text($roomNumber !== '' ? 492 : 286, 320, (string) ($input['subline'] ?? ''), 'subhead', $roomNumber !== '' ? 482 : 688, 74, 3, ['minFontSize' => 15, 'widthSafety' => 0.88]) .
                    ($direction !== 'none'
                ...
                $body =
                    ...
                    ($roomNumber !== ''
                        ? '<rect x="62" y="146" width="180" height="146" rx="28" class="tone-soft"/>' .
                            constructor_svg_render_fitted_text(94, 236, $roomNumber, 'subhead', 116, 94, 2, ['minFontSize' => 16, 'heightSafety' => 0.82])
                        : '') .
                    constructor_svg_render_fitted_text($locationX, 184, (string) ($input['location'] ?? ''), 'headline', $locationMaxWidth, 168, 4, ['minFontSize' => 20, 'widthSafety' => 0.84, 'heightSafety' => 0.86]) .
                    constructor_svg_render_fitted_text($locationX, 324, (string) ($input['subline'] ?? ''), 'subhead', $locationMaxWidth, 72, 3, ['minFontSize' => 15, 'widthSafety' => 0.88]) .
                    ($direction !== 'none'
        ...
        case 'information_stand':
            ...
            $sectionTwoTitleVisible = $sectionTwoTitle !== '';
            $sectionTwoBodyY = $footerY + ($sectionTwoTitleVisible ? $footerTitleHeight + ($compactStand ? 14.0 : 20.0) : ($compactStand ? 20.0 : 30.0));
            $sectionTwoBodyHeight = $sectionTwoTitleVisible
                ? $footerBodyHeight
                : max($compactStand ? 36.0 : 48.0, $footerHeight - ($compactStand ? 32.0 : 44.0));
            $body =
                constructor_svg_background_layers($backgroundStyle, $width, $height, $brandAssetBundle, $brandVariant, $designVariant, $graphicElement) .
                ...
                constructor_svg_render_fitted_text($margin, $margin + $headerHeight + ($compactStand ? 22.0 : 30.0), (string) ($input['headline'] ?? ''), $headlineClass, $width - ($margin * 2), max(44.0, $titleHeight * ($compactStand ? 0.56 : 0.66)), $compactStand ? 4 : 3, $headlineOptions) .
                constructor_svg_render_fitted_text($margin, $margin + $headerHeight + max($compactStand ? 54.0 : 74.0, $titleHeight * ($compactStand ? 0.72 : 0.78)), (string) ($input['subtitle'] ?? ''), $subtitleClass, $width - ($margin * 2), max(22.0, $titleHeight * ($compactStand ? 0.28 : 0.32)), $compactStand ? 3 : 2, $subtitleOptions) .
                ...
                ($sectionTwoTitleVisible
                    ? constructor_svg_render_fitted_text($margin + $infoCardWidth + $gap + 16.0, $footerY + ($compactStand ? 18.0 : 28.0), $sectionTwoTitle, $footerTitleClass, $infoCardWidth - 32.0, $footerTitleHeight, 2, $footerTitleOptions)
                    : '') .
                constructor_svg_render_fitted_text($margin + $infoCardWidth + $gap + 16.0, $sectionTwoBodyY, $sectionTwoBody, $footerBodyClass, $infoCardWidth - 32.0, $sectionTwoBodyHeight, $denseStand ? 3 : ($height >= 900 ? 5 : 4), $footerBodyOptions) .
                ($contactLine !== ''
                    ? constructor_svg_render_fitted_text($width / 2, $bottomNoteY + max(12.0, $bottomNoteHeight * 0.48), $contactLine, $contactClass, $width - ($margin * 2), $bottomNoteHeight, $compactStand ? 3 : 2, $contactOptions, 'middle')
                    : '');
            break;
        ...
        case 'room_navigation_sign':
            ...
            $body =
                ...
                constructor_svg_render_fitted_text(64, 162, (string) ($input['destination'] ?? ''), 'headline', $destinationWidth, 162, 4, ['minFontSize' => 20, 'widthSafety' => 0.84, 'heightSafety' => 0.86]) .
                constructor_svg_render_fitted_text(64, 298, (string) ($input['route_hint'] ?? ''), 'subhead', $destinationWidth, 82, 3, ['minFontSize' => 15, 'widthSafety' => 0.88]) .
                ($floorLabel !== ''
        ...
        case 'presentation_deck':
            ...
            $body =
                ...
                ($outlinePreview !== []
                    ? constructor_svg_render_text(96, 356, $outlinePreview, 'small', ['maxWidth' => 320, 'maxHeight' => 176, 'minFontSize' => 16, 'fill' => $lockupInk])
                    : '') .
                ...
                constructor_svg_render_fitted_text($contentX, 270, (string) ($input['title'] ?? ''), 'headline', $contentWidth, 316, 5, ['minFontSize' => 20, 'widthSafety' => 0.84, 'heightSafety' => 0.86]) .
                '<rect x="' . $contentX . '" y="476" width="' . max(640.0, $contentWidth) . '" height="164" rx="32" class="card"/>' .
                constructor_svg_render_fitted_text($contentX + 48.0, 548, $keyMessage, 'body', max(544.0, $contentWidth - 96.0), 112, 3, ['minFontSize' => 15]) .
                constructor_svg_render_fitted_text($contentX, 714, (string) ($input['speaker'] ?? ''), 'subhead', 420, 54, 2, ['minFontSize' => 16]) .
                constructor_svg_render_fitted_text($contentX, 764, (string) ($input['speaker_role'] ?? ''), 'body', 520, 70, 2, ['minFontSize' => 15]);
            break;
        ...
        case 'badge':
            ...
            $body =
                ...
                constructor_svg_render_fitted_text(76, 290, (string) ($input['event_name'] ?? $cityLabel), 'small', 560, 58, 2, ['minFontSize' => 15]) .
                ...
                constructor_svg_render_fitted_text(76, 796, (string) ($input['role'] ?? ''), 'subhead', 548, 118, 3, ['minFontSize' => 14, 'widthSafety' => 0.86, 'heightSafety' => 0.84]) .
        ...
        case 'social_post':
            ...
            $body =
                ...
                ($isMarkPost
                    ...
                    : '') .
                constructor_svg_render_fitted_text(80, 320, (string) ($input['headline'] ?? ''), 'headline', $width - 160, 356, 5, ['minFontSize' => 18, 'widthSafety' => 0.82, 'heightSafety' => 0.84]) .
                constructor_svg_render_fitted_text(80, 690, (string) ($input['message'] ?? ''), 'body', $width - 160, 470, 6, ['minFontSize' => 14, 'widthSafety' => 0.88]) .
                '<rect x="80" y="' . ($height - 178) . '" width="' . ($width - 160) . '" height="88" rx="28" class="card"/>' .
                constructor_svg_render_fitted_text(124, $height - 120, (string) ($input['cta'] ?? ''), 'subhead', $width - 280, 60, 3, ['minFontSize' => 13, 'widthSafety' => 0.88]) .
        ...
        case 'letterhead':
            ...
            $body =
                ...
                constructor_svg_render_fitted_text(84, 150, (string) ($input['department'] ?? ''), 'subhead', 620, 72, 2, ['minFontSize' => 14, 'fill' => $brandVariant === 'white' ? $lockupInk : 'var(--ctor-ink)']) .
                constructor_svg_render_fitted_text(84, 312, (string) ($input['document_title'] ?? ''), 'title', 1040, 214, 4, ['minFontSize' => 18, 'widthSafety' => 0.86, 'heightSafety' => 0.88]) .
                ...
                constructor_svg_render_fitted_text(900, 1480, (string) ($input['signer'] ?? ''), 'small', 240, 54, 2, ['minFontSize' => 12]);
            break;
        ...
        case 'rollup':
            ...
            $body =
                ...
                constructor_svg_render_fitted_text(88, 1360, (string) ($input['subline'] ?? ''), 'body', 824, 390, 7, ['minFontSize' => 14, 'widthSafety' => 0.88]) .
                '<rect x="88" y="1730" width="824" height="232" rx="34" class="card"/>' .
                constructor_svg_render_fitted_text(132, 1810, (string) ($input['event_name'] ?? ''), 'subhead', 500, 114, 3, ['minFontSize' => 16]) .
    ...
    return [
        ...
        'content' => constructor_svg_document($width, $height, $body, $theme, $lockupSurface),
        'previewType' => 'svg',
    ];
}
    ...
    return [
        ...
        'bullets' => array_values(array_filter([
            ...
            'Оформление: ' . implode(' • ', array_filter([
                (string) ($styleLabels['designVariantLabel'] ?? ''),
